...
describe('ChallengesService', () => {
  ...
  describe('getChallenge', () => {
    it('Camada 1 — deve retornar do pool Redis quando disponível', async () => {
      ...
      expect(mockPool.popQuestion).toHaveBeenCalledWith('vid-1');
    ...
    it('Camada 3 — deve retornar resposta hardcoded quando tabela estática também está vazia', async () => {
      mockPool.popQuestion.mockResolvedValue(null);
      mockPrisma.generatedQuestion.findFirst.mockResolvedValue(null);
  ...
  describe('pushQuestionsToPool', () => {
    ...
    it('deve lançar NotFoundException para vídeo inexistente', async () => {
      ...
      await expect(
        service.pushQuestionsToPool('bad-id', QUESTIONS),
    ...
    it('deve armazenar embedding via raw SQL quando fornecido com 1536 dimensões', async () => {
      mockPrisma.video.findUnique.mockResolvedValue({ id: 'vid-1' });
      mockPrisma.generatedQuestion.create.mockResolvedValue({
        id: 'db-q1',
        ...QUESTIONS[0],
      });
      ...
      await service.pushQuestionsToPool('vid-1', [{ ...QUESTIONS[0], embedding }]);
    ...
    it('deve ignorar embedding com dimensões incorretas', async () => {
      ...
      // Embedding com dimensão errada (512 em vez de 1536)
      await service.pushQuestionsToPool('vid-1', [
        { ...QUESTIONS[0], embedding: new Array(512).fill(0.1) },
      ]);
  ...
  describe('getPoolSize', () => {
    it('deve retornar o tamanho do pool com o videoId', async () => {
      ...
      const result = await service.getPoolSize('vid-1');
      expect(result).toEqual({ videoId: 'vid-1', size: 9 });
    });
  });
});
