...
export class ChallengesService {
  ...
  /**
   * Busca a pergunta mais recente não utilizada para o vídeo.
   * Em produção seria substituído por busca vetorial com <=> (cosine distance).
   */
  ...
  private async getStaticFallback(): Promise<Challenge> {
    ...
    if (count === 0) {
      // Última linha de defesa: resposta hardcoded
      return {
