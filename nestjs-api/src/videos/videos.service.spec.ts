...
describe('VideosService', () => {
  ...
  describe('getRecord', () => {
    ...
    it('deve lançar NotFoundException quando vídeo não existe', async () => {
      ...
      await expect(service.getRecord('nao-existe')).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('saveUploadedFile', () => {
    ...
    it('deve salvar arquivo no disco e persistir no banco', async () => {
      mockPrisma.video.create.mockResolvedValue({ ...DB_VIDEO, originalName: 'video-teste.mp4' });
      ...
      expect(mockPrisma.video.create).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ originalName: 'video-teste.mp4', mimeType: 'video/mp4' }),
        }),
      );
    });

    it('deve usar .mp4 como extensao padrao para arquivos desconhecidos', async () => {
      mockPrisma.video.create.mockResolvedValue({ ...DB_VIDEO, originalName: 'video.avi' });
      await service.saveUploadedFile({ ...FILE, originalname: 'video.avi' } as Express.Multer.File);
      const call = mockPrisma.video.create.mock.calls[0][0];
      expect(call.data.storedFilename).toMatch(/\.mp4$/);
    });

    it('deve usar extensao .webm para arquivos webm', async () => {
      mockPrisma.video.create.mockResolvedValue({ ...DB_VIDEO, originalName: 'video.webm' });
      await service.saveUploadedFile({ ...FILE, originalname: 'video.webm' } as Express.Multer.File);
      const call = mockPrisma.video.create.mock.calls[0][0];
      expect(call.data.storedFilename).toMatch(/\.webm$/);
    });
  });
});
