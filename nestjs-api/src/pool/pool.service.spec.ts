...
describe('PoolService', () => {
  ...
  describe('pushQuestions', () => {
    it('should push a single question and return pool size', async () => {
      ...
      expect(mockRedis.rpush).toHaveBeenCalledWith('pool:video:vid-1', JSON.stringify(QUESTION));
      expect(size).toBe(1);
    });
  ...
  describe('popQuestion', () => {
    ...
    it('should return null when pool is empty', async () => {
      ...
      expect(await service.popQuestion('vid-1')).toBeNull();
    });
  });
  ...
  describe('getPoolSize', () => {
    it('should return the Redis LLEN value', async () => {
      ...
      expect(await service.getPoolSize('vid-1')).toBe(7);
